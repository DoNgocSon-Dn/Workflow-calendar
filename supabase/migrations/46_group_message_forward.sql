-- Chuyển tiếp tin nhắn sang nhóm khác.
--
-- Chỉ cần một cột snapshot `forwarded_from_group` (tên nhóm nguồn tại thời điểm
-- chuyển) — nội dung/tệp được COPY sang tin mới, không trỏ tin gốc. 3 RPC
-- list/edit/delete phải trả thêm cột → drop rồi tạo lại (kế thừa 45).
--
-- Chạy 1 lần trong Supabase SQL Editor, SAU 45_group_message_pin.sql.
-- An toàn khi chạy lại.

alter table public.group_messages
  add column if not exists forwarded_from_group text;
...
returns table (
  ...
  pinned_at timestamptz, pinned_by uuid, forwarded_from_group text
)
language plpgsql
...
begin
  ...
  return query
    select gmsg.id, gmsg.group_id, gmsg.sender_id, gmsg.message, gmsg.created_at,
           ...
           gmsg.pinned_at, gmsg.pinned_by, gmsg.forwarded_from_group
